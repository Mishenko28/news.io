...
export default function News({ search, category }: { search?: string, category?: string }) {
    return (
        <div className="flex-1 flex flex-col gap-2 border">
            ...
            <div className="p-2">
                ...
                <Suspense key={`${search}${category}`} fallback={<NewsItemsSkeleton />}>
                    <NewsItems search={search} category={category} />
                </Suspense>
            </div>
        </div>
    )
